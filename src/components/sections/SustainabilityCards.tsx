'use client';

import { motion } from 'framer-motion';
import FadeUp from '@/components/animations/FadeUp';
import { IMAGE_URLS } from '@/lib/constants';
import { useT } from '@/i18n/LocaleProvider';

interface CardData {
  icon: string;
  iconColor: string;
  titleKey: string;
  descKey: string;
  bg: string;
  bgIcon: string;
}

const cards: CardData[] = [
  {
    icon: 'verified',
    iconColor: 'text-primary',
    bgIcon: IMAGE_URLS.iconQuality,
    titleKey: 'sustainability.qualityTitle',
    descKey: 'sustainability.qualityDesc',
    bg: 'bg-surface-container-low',
  },
  {
    icon: 'groups',
    iconColor: 'text-secondary',
    bgIcon: IMAGE_URLS.iconCommunity,
    titleKey: 'sustainability.communityTitle',
    descKey: 'sustainability.communityDesc',
    bg: 'bg-surface-container-high',
  },
  {
    icon: 'eco',
    iconColor: 'text-primary',
    bgIcon: IMAGE_URLS.iconOrganic,
    titleKey: 'sustainability.organicTitle',
    descKey: 'sustainability.organicDesc',
    bg: 'bg-surface-container-low',
  },
];

export default function SustainabilityCards() {
  const t = useT();
  return (
    <section className="bg-surface min-h-screen flex items-center py-32 overflow-hidden">
      <div className="mx-auto max-w-content px-[clamp(1.25rem,5vw,6rem)]">
        {/* Header */}
        <div className="mb-16 text-center">
          <FadeUp>
            <p className="mb-4 text-xs font-semibold uppercase tracking-[0.4em] text-primary">
              {t('sustainability.overline')}
            </p>
          </FadeUp>
          <FadeUp delay={0.1}>
            <h2 className="font-headline text-h2 font-light italic text-on-surface">
              {t('sustainability.title')}
            </h2>
          </FadeUp>
          <FadeUp delay={0.2}>
            <p className="mx-auto mt-4 max-w-xl font-light leading-relaxed text-on-surface-variant">
              {t('sustainability.description')}
            </p>
          </FadeUp>
        </div>

        {/* Cards */}
        <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
          {cards.map((card, index) => (
            <FadeUp key={card.titleKey} delay={0.1 * index}>
              <motion.div
                initial="initial"
                whileInView="whileInView"
                viewport={{ once: true, margin: "-100px" }}
                whileHover="hover"
                className={`group relative flex flex-col items-start rounded-sm p-10 text-left transition-all duration-300 ${
                  card.bg
                } ${index === 1 ? 'md:-translate-y-2' : ''}`}
              >
                {/* Background Icon Watermark */}
                <motion.div
                  variants={{
                    initial: { opacity: 0, scale: 0.8, x: 20, y: -20 },
                    whileInView: { opacity: 0.15, scale: 1, x: 0, y: 0 },
                    hover: { opacity: 0.3, scale: 1.1, rotate: -5 },
                  }}
                  transition={{ duration: 1, ease: "easeOut" }}
                  className="absolute -top-2 -right-2 w-32 h-32 pointer-events-none"
                >
                  <img src={card.bgIcon} alt="" className="w-full h-full object-contain filter grayscale" />
                </motion.div>

                <h3 className="relative z-10 font-headline text-2xl font-light text-on-surface">
                  {t(card.titleKey)}
                </h3>

                <p className="relative z-10 mt-4 text-base font-light leading-relaxed text-on-surface-variant max-w-[85%]">
                  {t(card.descKey)}
                </p>
              </motion.div>
            </FadeUp>
          ))}
        </div>
      </div>
    </section>
  );
}
